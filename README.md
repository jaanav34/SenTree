...
SenTree is a climate-risk simulation and search platform built around a graph neural network pipeline. It ingests ISIMIP-style climate data, identifies tail-risk escalation zones, simulates adaptation interventions, renders MP4 heatmaps, computes resilience ROI, and exposes the results in a Streamlit dashboard with optional semantic video search.
...
- Simulates interventions such as mangrove restoration and regenerative agriculture.
- Computes resilience ROI, loss avoided, and uncertainty-adjusted ranges.
- Renders climate-risk and intervention comparison videos.
- Indexes generated videos into ChromaDB using Gemini embeddings for semantic retrieval.
...
- Python 3.12 or 3.13 is the intended target.
- `ffmpeg` must be available on `PATH` to render MP4 outputs.
- A Gemini API key is required only for semantic video indexing and query embedding.

Python packages are listed in [`requirements.txt`](/Users/prathampatel/PycharmProjects/SenTree/requirements.txt).
...
- renders MP4 videos and a tail-risk map
...
- Synthetic mode: generated by [`data/generate_synthetic.py`](/Users/prathampatel/PycharmProjects/SenTree/data/generate_synthetic.py), producing a 2015-2100 Southeast Asia coastal dataset with temperature, precipitation, GDP, population, soil moisture, and coastal factor grids.
- ISIMIP mode: if matching `ssp370 tas` NetCDF files are found in `data/raw`, the loader subsets them to Southeast Asia, resamples daily data to annual means, converts Kelvin to Celsius, and synthesizes missing precipitation and socioeconomic proxy layers.

The loader logic lives in [`src/data/load_isimip.py`](/Users/prathampatel/PycharmProjects/SenTree/src/data/load_isimip.py).
...
- `outputs/videos/comparison_mangrove_restoration.mp4`
- `outputs/videos/comparison_regenerative_agriculture.mp4`
...
The Streamlit app in [`src/dashboard/app.py`](/Users/prathampatel/PycharmProjects/SenTree/src/dashboard/app.py) includes:
...
- The repo enforces use of a virtual environment through [`sentree_venv.py`](/Users/prathampatel/PycharmProjects/SenTree/sentree_venv.py).
- `ffmpeg` is required for MP4 generation. Without it, rendering will fail.
- Gemini embedding requires `GOOGLE_API_KEY` or `GEMINI_API_KEY`.
