...
TIMINGS_OUTPUT_PATH = "outputs/roi/pipeline_timings.json"
...
def _timed(timings: dict, label: str):
    ...
    try:
        ...
        timings[label] = float(time.perf_counter() - start)
...
def _print_timing_summary(timings: dict) -> None:
    ...
    for label, seconds in sorted(timings.items(), key=lambda kv: kv[1], reverse=True):
        print(f"{label:<38} {_format_seconds(seconds):>10}")
    print("=" * 60)
...
with _timed(timings, "1) load_climate_data"):
...
with _timed(timings, "1b) koppen_geiger (cache/load)"):
    try:
...
with _timed(timings, "2) compute_tail_risk"):
...
with _timed(timings, "4) train_gnn"):
...
print("\n[6/7] Computing ROI...")
roi_results = {}
with _timed(timings, "6) compute_roi (all interventions)"):
...
with _timed(timings, "7d) compute_tail_risk_series"):
...
with _timed(timings, "7f) opportunity_map (compute+render)"):
...
try:
    os.makedirs(os.path.dirname(TIMINGS_OUTPUT_PATH), exist_ok=True)
    timing_payload = {
        "slurm_job_id": os.environ.get("SLURM_JOB_ID"),
        "region": region,
        "coarsen": int(coarsen),
        "intervention_strength": float(intervention_strength),
        "kg_workers": int(os.environ.get("SENTREE_KG_WORKERS", "1") or "1"),
