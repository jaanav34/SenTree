...
st.markdown(
    ...
    .sentree-hero-logo img {
        ...
        height: auto;
        border-radius: 14px;
        border: 1px solid rgba(23, 52, 47, 0.18);
    ...
    button[kind="tab"] {
        border-radius: 999px;
        border: 1px solid rgba(23, 52, 47, 0.14);
        background: rgba(255, 252, 246, 0.86);
        padding: 0.46rem 1.05rem;
        font-weight: 700;
        color: #26443d;
        box-shadow: 0 4px 12px rgba(23, 52, 47, 0.06);
    }

    button[kind="tab"][aria-selected="true"] {
        background: linear-gradient(135deg, #17342f, #1e5c53);
        color: #f8f4ea !important;
        border-color: rgba(23, 52, 47, 0.22);
        box-shadow: 0 10px 24px rgba(23, 52, 47, 0.2);
    }
    ...
    .sentree-context {
        ...
        border-radius: 16px;
        background: linear-gradient(180deg, rgba(255, 251, 245, 0.96), rgba(242, 248, 244, 0.93));
    ...
    .sentree-overview-item {
        border-radius: 14px;
        border: 1px solid rgba(23, 52, 47, 0.11);
...
_SUB_PAGES = {
    "Overview":       None,                                        # no sub-pages
    "Recommendation": ["Brief", "Comparison"],
    "Evidence":       ["Search", "Videos", "Risk Over Time", "Map"],
    "Model":          ["GNN Playback", "Math Foundations"],
}

with st.sidebar:
    st.markdown(
        """
        <div style="padding:0.2rem 0 1.2rem; border-bottom:1px solid rgba(237,232,216,0.14); margin-bottom:1.1rem;">
            <div style="font-size:0.68rem;font-weight:900;letter-spacing:0.13em;text-transform:uppercase;
                        color:rgba(237,232,216,0.55);font-family:'DM Mono',monospace;margin-bottom:0.55rem;">
                Navigation
            </div>
        </div>
        """,
    ...
    # Sub-page selector (only shown when section has sub-pages)
    active_sub = None
    sub_pages = _SUB_PAGES.get(active_section)
    if sub_pages:
        st.markdown(
            """<div style="height:1px;background:rgba(237,232,216,0.12);margin:0.9rem 0 0.8rem;"></div>""",
            unsafe_allow_html=True,
        )
        active_sub = st.radio(
            "View",
            sub_pages,
            key=f"sentree_nav_sub_{active_section}",
            label_visibility="collapsed",
        )

    st.markdown(
        """<div style="height:1px;background:rgba(237,232,216,0.12);margin:1.1rem 0 1rem;"></div>""",
        unsafe_allow_html=True,
    )

    # Capital allocation slider lives permanently in sidebar
    capital_allocation_m = st.slider(
        ...
        help="Adjust capital to see ROI and loss avoided update with a diminishing-returns assumption.",
    )

    st.markdown(
        f"""
        <div style="margin-top:0.6rem;padding:0.7rem 0.85rem;border-radius:14px;
                    background:rgba(237,232,216,0.07);border:1px solid rgba(237,232,216,0.12);">
            <div style="font-size:0.68rem;font-weight:900;letter-spacing:0.10em;text-transform:uppercase;
                        color:rgba(237,232,216,0.45);font-family:'DM Mono',monospace;margin-bottom:0.45rem;">
                Context
            </div>
...
st.markdown(
    ...
    section[data-testid="stSidebar"] div[data-testid="stRadio"] label {
        ...
        color: rgba(237,232,216,0.80) !important;
        cursor: pointer;
        transition: background 0.18s ease, color 0.18s ease;
    }
    section[data-testid="stSidebar"] div[data-testid="stRadio"] label:hover {
        background: rgba(13,148,136,0.18) !important;
        color: rgba(237,232,216,1) !important;
    }
...
# Breadcrumb strip
# ---------------------------------------------------------------------------
_sub_label = f" › {active_sub}" if active_sub else ""
st.markdown(
    f"""
    <div style="
        ...
        border-radius:12px;
        background:rgba(15,42,36,0.05);
        border:1px solid rgba(15,50,42,0.08);
        font-size:0.80rem;font-weight:700;
        color:#4a6159;font-family:'DM Mono',monospace;letter-spacing:0.02em;
    ">
        <span style="color:var(--sentree-accent);">{_NAV_SECTIONS[active_section]}</span>
        <span style="color:rgba(74,97,89,0.4);">/</span>
        <span style="color:#0f2a24;">{active_section}</span>
        {'<span style="color:rgba(74,97,89,0.4);">/</span><span style="color:#0f2a24;">' + active_sub + '</span>' if active_sub else ''}
    </div>
    """,
    unsafe_allow_html=True,
)
...
# ---------------------------------------------------------------------------
# Load shared data needed by multiple pages
# ---------------------------------------------------------------------------
...
# ── RECOMMENDATION ──────────────────────────────────────────────────────────
elif active_section == "Recommendation":
...
# ── EVIDENCE ────────────────────────────────────────────────────────────────
    ...
    elif active_sub == "Risk Over Time":
        section_header(
            "Monitor",
            "Risk over time",
...
# ── MODEL ────────────────────────────────────────────────────────────────────
        ...
        if playback_mode == "Embedded React app":
            ...
            if training is None:
                ...
                epoch_selected = control_cols[0].slider(
                    "Epoch",
                    min_value=1, max_value=total_epochs,
                    value=st.session_state.training_epoch_idx + 1,
                    disabled=st.session_state.training_playing,
                    key="training_epoch_slider",
                )
                play_btn  = control_cols[1].button("Resume" if st.session_state.training_playing else "Play",  use_container_width=True, key="training_play_btn")
                ...
                render_training_frame(
                    viz_placeholder, metrics_placeholder, training,
                    st.session_state.training_epoch_idx,
                    show_edges=True, highlight_targets=highlight_targets,
                )
    ...
    elif active_sub == "Math Foundations":
        render_math_view()
